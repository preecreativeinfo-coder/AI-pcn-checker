...
// POST /api/analyze — AI analysis of a PCN's likelihood of a successful
// challenge, using the Claude API. The API key stays server-side.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!apiKey) {
    res.status(503).json({
      error:
        "ANTHROPIC_API_KEY is not configured. Add it in the project environment variables to enable AI Analysis.",
    });
    return;
  }
